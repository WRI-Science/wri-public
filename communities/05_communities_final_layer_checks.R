...
#communities_resistance_only <- app(communities_stack, fun = mean, na.rm = TRUE)
# do calc like this because communities status is a mask and we are using masked versions:
...
results <- foreach(i = 1:length(states_vect), .packages = c("terra", "sf")) %dopar% {
  ...
  cropped <- crop(communities_stack, state_geom)
...
writeRaster(merged_raster,
            file.path(out_folder, "communities_classified_merged.tif"),
            overwrite = TRUE)