...
# resistance
drought_plans <- rast(file.path(wri_project_root, "final_layers", "2024", "water", "indicators", "drought_plan_scores.tif"))
drought_plans <- align_raster_to_template(study_area_rast, drought_plans)
water_treatment <- rast(file.path(wri_project_root, "final_layers", "2023", "water", "indicators", "water_treatment_scores_2024.tif"))
...
# status
status <- rast(file.path(wri_project_root, "final_layers", "2024", "water", "indicators", "streamflow_status_scores_2024.tif"))
status

# calculate domain score
# multiply status by resilience (resistance in this case)

# resilience <- rast(file.path(wri_project_root, "final_layers", "water", "water_resilience.tif"))
# status <- rast(file.path(wri_project_root, "final_layers", "water", "water_status.tif"))
...
writeRaster(resistance_aligned, file.path(wri_project_root, "final_layers", "2024", "water", "water_resistance.tif"), overwrite = TRUE)
# write out resilience score
writeRaster(resilience_aligned, file.path(wri_project_root, "final_layers", "2024", "water", "water_resilience.tif"), overwrite = TRUE)
# write out status score
writeRaster(status_aligned, file.path(wri_project_root, "final_layers", "2024", "water", "water_status.tif"), overwrite = TRUE)
# write out domain score
writeRaster(domain_score, file.path(wri_project_root, "final_layers", "2024", "water", "water_domain_score_mean.tif"), overwrite = TRUE)
...
# writeRaster(resistance_90m, file.path(wri_project_root, "domains", "water", "resistance_scores_90m.tif"), overwrite = TRUE)
# resilience90 <- rast(resilience)
# res(resilience90) <- 90
# resilience_90m <- project(resilience, resilience90, method = "bilinear")
# writeRaster(resilience_90m, file.path(wri_project_root, "domains", "water", "resilience_scores_90m.tif"), overwrite = TRUE)
# status90 <- rast(status)
# res(status90) <- 90
# status_90m <- project(status, status90, method = "bilinear")
# writeRaster(status_90m, file.path(wri_project_root, "domains", "water", "status_scores_90m.tif"), overwrite = TRUE)
# 
...
# writeRaster(domain_score_90m, file.path(wri_project_root, "domains", "water", "domain_score_90m.tif"), overwrite = TRUE)
# 
# 
# 
# 
...
# 
# writeRaster(domain_score_aligned_0_100, file.path(wri_project_root, "final_layers", "water", "water_domain_score_mean.tif"), overwrite = TRUE)
