...
# reproject to Albers (EPSG:5070), rescale to a 0–1 “higher = worse” metric,
# rasterize to a 90 m study‑area template, and export the final GeoTIFF.
...
#### Data Layers ####
us_days_above_100_2024 <- read_csv(file.path(intermediate_data_file_path, "aqi/us_days_above_100_2024.csv"))
canada_days_above_100_2024 <- read_csv(file.path(intermediate_data_file_path, "aqi/canada_days_above_100_2024.csv"))

#### Combine the US and Canada count of days > AQI 100 #### 

# Define the function to combine and process the data frames
combine_aqi_data <- function(us_df, canadian_df) {
  ...
  canadian_df <- canadian_df %>%
    dplyr::select(defining_site, local_site_name, owning_agency, latitude, longitude, days_above_100)
  
  us_df <- us_df %>%
    dplyr::select(defining_site, county_name, local_site_name, owning_agency, latitude, longitude, days_above_100)
...
# Run the function
us_can_days_above_100_combined_2024 <- combine_aqi_data(us_days_above_100_2024, canada_days_above_100_2024)

# Rescale values and move geometry column
us_can_days_above_100_combined_2024 <- us_can_days_above_100_combined_2024 %>%
  mutate(
    days_above_100_rescaled = pmax(0, pmin(1, scales::rescale(days_above_100, to = c(1, 0), from = c(0, 52)))))

# # plot sensor coordinates in 4326
# uncomment to check
# ggplot(us_can_days_above_100_combined_2024, aes(x = longitude, y = latitude)) +
...
# read in days above 100 aqi and transform to mollweide
us_can_days_above_100_2024_moll <- us_can_days_above_100_combined_2024 %>%
...
sf::st_write(
  us_can_days_above_100_2024_moll,
  dsn = file.path(intermediate_data_file_path, "us_can_days_above_100_combined_2024_moll.shp"),
  delete_dsn = TRUE,
  quiet = FALSE
)
...
aqi_interp_list <- foreach(chunk = grid_chunks, .packages = c("sf", "gstat", "dplyr")) %dopar% {
  idw_chunk(chunk, us_can_days_above_100_2024_moll)
}
...
# aqi_interp_5070_cropped_rescaled <- aqi_interp_5070_cropped %>%
#   mutate(
#     days_above_100_rescaled = pmax(0, pmin(1, scales::rescale(aqi, to = c(1, 0), from = c(0, 52))))
#   )
...
air_quality_aqi_100_2024 <- terra::rasterize(aqi_interp_5070_cropped_rescaled, 
                                                   study_area_90m_5070, 
                                                       field = "aqi_rescaled_0_1", 
                                                       fun = "mean")

# Plot the final raster
plot(air_quality_aqi_100_2024, main = "Status: Days above AQI 100")

# Align indicator with study_area_90m_template raster
air_quality_aqi_100_2024 <- align_raster_to_template(study_area_90m_5070, air_quality_aqi_100_2024, input_type = "continuous")

# Save to aurora
writeRaster(air_quality_aqi_100_2024, 
            filename = file.path(final_layers_file_path, "indicators/air_quality_status_aqi_100.tif"),
            overwrite = TRUE)
